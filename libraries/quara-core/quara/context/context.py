...
from quara.api.brokers import Broker, BrokerContext
from quara.api.databases import Database, DatabaseContext
from quara.api.schedulers import Scheduler, SchedulerContext
from quara.api.storages import Storage, StorageContext
from quara.core.dicts import deep_update, dict_from_yaml

from .vars import broker, database, scheduler, storage
...
class Context:
    ...
    def set_contextvars(self) -> None:
        ...
        if enabled:
            logger.debug("Enabling database")
        ...
        enabled = options.pop("enabled")
        if enabled:
            logger.debug("Enabling message broker")
            backend = options.pop("backend")
        ...
        enabled = options.pop("enabled")
        if enabled:
            logger.debug("Enabling object storage")
            backend = options.pop("backend")
